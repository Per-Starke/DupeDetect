...
def printFileStatistics(dom, waypoints):
    ...
    "File Statistics:"
    print
    'file %s contains %d waypoints, %d caches and %d logs.' % (filename, waypoints.length, nrOfCaches, nrOfLogs)
    print
    '=' * 70, "\n"
...
def printWayPoint(wpt, nrOfLogs):
    ...
    '-' * 20
    print
    "Waypoint %s contains %d logs" % (wname, nrOfLogs)
    print
    "    cache: %s " % (cacheName)
    print
    '    desc: ', cacheDesc
    print
    "    lat: %s, lon: %s " % (wpt.attributes['lat'].value, wpt.attributes['lon'].value)
...
def printLogEntry(logEntry):
    ...
    '     ', '- ' * 5
    print
    ' ' * 8, 'id = %s, %s, logdate = %s by %s, %s' % (logId, logType, logDate, finder, logText)
